...
package org.keycloak.social.baidu;
...
public class BaiduIdentityProvider extends AbstractOAuth2IdentityProvider implements SocialIdentityProvider {
	...
	protected BrokeredIdentityContext doGetFederatedIdentity(String accessToken) {
		try {
			JsonNode profile = SimpleHttp.doGet(PROFILE_URL+"?access_token="+accessToken, session).header("Authorization", "Bearer " + accessToken).asJson();
                        logger.error("token:"+accessToken+"  profile: " + profile);
